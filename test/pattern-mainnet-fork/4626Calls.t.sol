...
contract SUSDSTestBase is ForkTestBase {
    ...
    function setUp() override public {
        ...
        // Note: This value varies by fork block. At block in use, no drip occurs.
        SUSDS_DRIP_AMOUNT = 0;

        assertEq(SUSDS_CONVERTED_ASSETS, 1091428409112860274, "SUSDS_CONVERTED_ASSETS");
        assertEq(SUSDS_CONVERTED_SHARES, 916230502752649142, "SUSDS_CONVERTED_SHARES");

        assertEq(SUSDS_TOTAL_ASSETS, 6375996661708864077586962201, "SUSDS_TOTAL_ASSETS");
        assertEq(SUSDS_TOTAL_SUPPLY, 5841882626906725130696796098, "SUSDS_TOTAL_SUPPLY");
    }
...
contract MainnetControllerDepositERC4626Tests is SUSDSTestBase {
    ...
    function test_depositERC4626() external {
        ...
        assertEq(usds.balanceOf(address(almProxy)),          1e18, "usds.balanceOf(address(almProxy))");
        assertEq(usds.balanceOf(address(mainnetController)), 0, "usds.balanceOf(address(mainnetController))");
        assertEq(usds.balanceOf(address(susds)),             USDS_BAL_SUSDS, "usds.balanceOf(address(susds))");

        assertEq(usds.allowance(address(buffer),   address(vault)),  type(uint256).max);
        assertEq(usds.allowance(address(almProxy), address(susds)),  0, "usds.allowance(address(almProxy), address(susds))");

        assertEq(susds.totalSupply(),                SUSDS_TOTAL_SUPPLY, "susds.totalSupply()");
        assertEq(susds.totalAssets(),                SUSDS_TOTAL_ASSETS, "susds.totalAssets()");
        assertEq(susds.balanceOf(address(almProxy)), 0, "susds.balanceOf(address(almProxy))");

        vm.prank(relayer);
        uint256 shares = mainnetController.depositERC4626(address(susds), 1e18);

        assertEq(shares, SUSDS_CONVERTED_SHARES, "shares");

        assertEq(usds.balanceOf(address(almProxy)),          0, "usds.balanceOf(address(almProxy))");
        assertEq(usds.balanceOf(address(mainnetController)), 0, "usds.balanceOf(address(mainnetController))");
        assertEq(usds.balanceOf(address(susds)),             USDS_BAL_SUSDS + SUSDS_DRIP_AMOUNT + 1e18, "usds.balanceOf(address(susds))");

        assertEq(usds.allowance(address(buffer),   address(vault)), type(uint256).max, "usds.allowance(address(buffer),   address(vault))");
        assertEq(usds.allowance(address(almProxy), address(susds)), 0, "usds.allowance(address(almProxy), address(susds))");

        assertEq(susds.totalSupply(),                SUSDS_TOTAL_SUPPLY + shares, "susds.totalSupply()");
        assertEq(susds.totalAssets(),                SUSDS_TOTAL_ASSETS + 1e18, "susds.totalAssets()");
        assertEq(susds.balanceOf(address(almProxy)), SUSDS_CONVERTED_SHARES, "susds.balanceOf(address(almProxy))");
    }
